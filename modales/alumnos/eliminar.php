...
if (!$nC || !$alumno) {
    echo json_encode(['exito' => false, 'mensaje_final' => 'Número de control no proporcionado o inválido.']);
    exit;
}
$conn->begin_transaction();
try {
    // Comprobar si el alumno está inscrito
    $queryInscripcion = "SELECT fol FROM inscripciones WHERE nC = ?";
    $stmtInscripcion = $conn->prepare($queryInscripcion);
    $stmtInscripcion->bind_param("s", $nC);
    $stmtInscripcion->execute();
    $stmtInscripcion->store_result();
    if ($stmtInscripcion->num_rows > 0) {
        throw new Exception("El alumno se encuentra inscrito, tiene que darlo de baja para eliminar su información.");
    }
