...
if ($_SERVER['REQUEST_METHOD'] !== 'GET') {
    die("Error: Datos en modo GET.");
}
// Validar y procesar los datos recibidos del formulario
$fo = $_GET['foMod']; // Folio de pago
$fe = $_GET['feMod']; // Fecha de pago
$cN = $_GET['cNMod']; // Clave del nivel
$cC = isset($_GET['cCMod']) ? trim($_GET['cCMod']) : null; // Clave de la carrera
$nC = $_GET['nCA']; // Número de control
$cT = $_GET['tpa']; // Clave tipo de pago
$de = $_GET['deMod']; // Descripción
$ca = $_GET['caMod']; // Cantidad
$im = $_GET['imMod']; // Importe
$tot = $_GET['totMod']; // Total
$cF = $_GET['formaPagoMod']; // Clave forma de pago
...
try {
    ...
    if (!$nivel) {
        die("Error: No se encontró el nivel");
    }
    $stmtNivel->close();
    ...
    if (!empty($cC)) {
        ...
        if (!$carrera) {
            die("Error: No se encontró la carrera '$nivel'");
        }
        $stmtCarrera->close();
    } else {
        $carrera = "";
    ...
    if (!$alumno) {
        die("Error: No se encontró el alumno");
    }
    $stmtAlumno->close();
    ...
    if (!$tiPa) {
        die("Error: No se encontró el tipo de pago");
    }
    $stmtTipo->close();
    ...
    if (!$foPa) {
        die("Error: No se encontró la forma de pago");
    }
    $stmtForma->close();
} catch (Exception $e) {
